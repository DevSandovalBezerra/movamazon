...
try {
    ...
    // Toggle status
    $novoStatus = $termo['ativo'] ? 0 : 1;
    $acao = $novoStatus ? 'ativado' : 'desativado';

    $stmt = $pdo->prepare("UPDATE termos_eventos SET ativo = ? WHERE id = ?");
    $stmt->execute([$novoStatus, $termoId]);

    echo json_encode([
        'success' => true,
        'message' => "Termo $acao com sucesso",
        'novo_status' => $novoStatus
    ]);
} catch (Exception $e) {
    error_log('Erro ao alterar status do termo: ' . $e->getMessage());
