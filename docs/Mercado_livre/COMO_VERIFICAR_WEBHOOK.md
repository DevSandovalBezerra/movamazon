...
No fluxo atual, o `external_reference` é **imutável** (`MOVAMAZON_<ID>`) e o `payment_id` é registrado em `pagamentos_ml` já na criação (PIX/Boleto/Cartão). O webhook busca a inscrição pelo `external_reference` que vem no payload e, quando necessário, tenta fallback por `payment_id`.

**Correção aplicada no código:** o webhook mantém a busca por `external_reference` e inclui fallback por `payment_id`, garantindo que a inscrição seja localizada e atualizada para pago mesmo em cenários de inconsistência.
