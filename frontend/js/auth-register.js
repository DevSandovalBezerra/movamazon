...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!form) {
        console.error('FormulÒ¡rio nÒ£o encontrado');
        return;
    }
    ...
    form.addEventListener('submit', function(e) {
        e.preventDefault();
        console.log('FormulÒ¡rio submetido');
        
        // Limpar mensagens anteriores
        msgDiv.innerHTML = '';
        msgDiv.className = '';
        
        // Validar confirmaÒ§Ò£o de senha
        const senha = form.querySelector('[name="senha"]').value;
        const confirmarSenha = form.querySelector('[name="confirmar_senha"]').value;
        
        console.log('Validando senhas...');
        if (senha !== confirmarSenha) {
            showMessage('As senhas nÒ£o coincidem.', 'error');
            return;
        }
        
        // Desabilitar botÒ£o e mostrar loading
        ...
        // Coletar dados do formulÒ¡rio
        const formData = new FormData(form);
        
        // Remover campo de confirmaÒ§Ò£o de senha (nÒ£o enviar para API)
        formData.delete('confirmar_senha');
        
        // Log dos dados sendo enviados
        console.log('Dados do formulÒ¡rio:');
        ...
        .catch(error => {
            console.error('Erro na requisiÒ§Ò£o:', error);
            Swal.fire({
                icon: 'error',
                title: 'Erro!',
                text: 'Erro ao processar formulÒ¡rio. Tente novamente.'
            });
        })
        .finally(() => {
            // Reabilitar botÒ£o
