...
async function inicializarPagamento() {
    if (typeof USE_CHECKOUT_PRO_REDIRECT !== 'undefined' && USE_CHECKOUT_PRO_REDIRECT) {
        console.log(' Checkout Pro redirect ativo; inicializarPagamento não executa Brick.');
        return;
    }
    console.log(' ============================================');
    ...
    if (window.paymentBrickController) {
        console.log('  Payment Brick já foi inicializado anteriormente');
    ...
    try {
        ...
        if (brickContainer) {
            ...
            if (!isVisible) {
                console.warn('  Container não está visível, forçando visibilidade...');
...
function setupEventListeners() {
    ...
    if (btnPagar && btnPagar.hasAttribute('data-listener-added')) {
        console.log('  Botão já tem listener, removendo e recriando...');
...
//  EXPORTAR TODAS AS FUN PARA WINDOW (após todas as definiç
...
console.log(' ============================================');
console.log(' TODAS AS FUN EXPORTADAS PARA WINDOW');
console.log(' ============================================');
console.log(' Funç disponíveis:', {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (errorLower.includes('cpf') || errorLower.includes('documento') || (temCamposFaltantes && errorData.campos_faltantes.includes('cpf'))) {
                console.log('  [GERAR_PIX] CPF faltando, coletando...');
                try {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (errorLower.includes('cpf') || errorLower.includes('documento')) {
                console.log('  [GERAR_BOLETO] CPF faltando, coletando...');
                try {
                    await coletarCPFModal();
                    // Tentar novamente após coletar CPF
            ...
            if (erroEndereco) {
                console.log('  [GERAR_BOLETO] ============================================');
                console.log('  [GERAR_BOLETO] ENDERE FALTANDO DETECTADO!');
                console.log('  [GERAR_BOLETO] Mensagem:', errorMessage);
                console.log('  [GERAR_BOLETO] Campos faltantes:', errorData.campos_faltantes);
                console.log('  [GERAR_BOLETO] Abrindo modal para coletar endereço...');
                console.log('  [GERAR_BOLETO] ============================================');
                ...
                try {
                    ...
                    if (error.message === 'Endereço não informado') {
                        console.log('  [GERAR_BOLETO] Usuário cancelou coleta de endereço');
                        return; // Usuário cancelou
                    }
        ...
        if (!result.barcode || result.barcode === '') {
            console.warn(' [GERAR_BOLETO] ATENÇÃO: Barcode não foi retornado pela API!');
            console.warn('  [GERAR_BOLETO] Dados recebidos:', JSON.stringify(result, null, 2));
        } else {
...
//  Re-exportar funç no final (garantir que estão todas disponíveis)
window.setupEventListeners = setupEventListeners;
window.atualizarBotaoPagamento = atualizarBotaoPagamento;
window.inicializarPagamento = inicializarPagamento;
window.validarDadosParaPagamento = validarDadosParaPagamento;
