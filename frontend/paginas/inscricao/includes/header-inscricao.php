<?php
// ? CR?TICO: Verificar se sess?o j? foi iniciada antes de tentar iniciar novamente
// N?o usar @session_start() pois pode mascarar problemas reais
if (session_status() === PHP_SESSION_NONE) {
    // Se output buffering n?o estiver ativo, iniciar
    if (!ob_get_level()) {
        ob_start();
    }
    session_start();
...
<head>
    ...
    <title><?php echo isset($page_title) ? $page_title : 'Inscri??o - MovAmazon'; ?></title>
...
<style>
    ...
    /* Layout da Inscri??o - Estilo Modal */
    .inscricao-container {
        min-height: 100vh;
        background: #f8f9fa;
        padding: 20px;
                ...
                <?php if (isset($_SESSION['user_id'])): ?>
                    ...
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center space-x-2 text-gray-300 hover:text-brand-yellow transition-colors duration-200">
                            ...
                            <span class="hidden sm:block text-sm"><?php echo htmlspecialchars($_SESSION['user_name'] ?? 'Usu?rio'); ?></span>
        ...
        <div id="mobile-menu-inscricao" class="md:hidden hidden bg-brand-green border-t border-white/10">
            <div class="py-3 px-4 space-y-2">
                ...
                <a href="../public/index.php" class="block px-3 py-2 text-gray-100 hover:text-brand-yellow rounded-lg">
                    <i class="fas fa-home mr-2"></i>P?gina Inicial
                </a>
            </div>
        </div>
    </nav>
