...
-- =========================
-- Helper: executar ALTER se coluna não existir (MySQL 5.7)
-- =========================
SET @db := DATABASE();
...
-- =========================
-- 2) ALTER TABLES EXISTENTES (com checagem)
-- =========================
...
-- FK planos_treino_gerados -> assessorias / programas
-- (só cria se ainda não existir)
SET @fk := 'fk_planos_assessoria';
SET @sql := (SELECT IF(
  (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
   WHERE CONSTRAINT_SCHEMA=@db AND TABLE_NAME='planos_treino_gerados' AND CONSTRAINT_NAME=@fk)=0,
...
-- FK treinos -> assessorias / programas / usuarios (criado_por)
SET @fk := 'fk_treinos_assessoria';
SET @sql := (SELECT IF(
  (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
   WHERE CONSTRAINT_SCHEMA=@db AND TABLE_NAME='treinos' AND CONSTRAINT_NAME=@fk)=0,
  'ALTER TABLE `treinos`
...
SET @sql := (SELECT IF(
  (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
   WHERE CONSTRAINT_SCHEMA=@db AND TABLE_NAME='progresso_treino' AND CONSTRAINT_NAME=@fk)=0,
...
-- =========================
-- 3) RBAC: adicionar papeis (sem depender de IDs fixos)
-- =========================
-- Seu dump não trouxe INSERT de `papeis`, então vamos inserir por nome se não existir.
-- Isso funciona mesmo que já existam papéis no banco.
